...
> - Spectator permission isolation uses zero-tolerance judgment: accepted spectator-originated combat events = `0`, spectator-caused HP / Shield / Result mutations = `0`.
> - Spectator UI/state sync target: HP / Shield / Dead / Result display deltas relative to player clients stay within `300 ms`.
> - Mid-join or reconnect passes if Spectator C reaches the current match snapshot within `2 s`, and post-match reconnect reaches the correct result screen within `3 s`.
> - Spectator shared-space observation target once available: relative placement error <= `20 cm`, and stationary drift <= `10 cm` over `30 s`.
> - Stability cases pass only if the session completes with crash count = `0`, duplicate spectator instance count = `0`, and identity-mapping errors = `0`.
---
